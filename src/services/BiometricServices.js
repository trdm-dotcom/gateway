...
async function biometricRegister(messageId, req, res) {
  ...
  if (results.length > 0) {
    ...
    Logger.info('already exist biometric register for account, start unactivated this biometric ');
  ...
  if (config.enableEncryptPassword === true) {
    req.body['password'] = rsaEncrypt(req.body['password'], config.key.rsa.privateKey);
  }
...
async function biometricLogin(req) {
  ...
  Utils.validate(request.body['username'], 'username').setRequire().throwValid(invalidParams);
  invalidParams.throwErr();
  req.body['username'] = req.body['username'].trim();
  ...
  if (!verify.verify(publicKey, signature, 'base64')) {
    throw new Errors.GeneralError('LOGIN_BIOMETRIC_SIGNATURE_VERIFICATION_FAILED');
  }
}
...
async function queryBiometricStatus(req, res) {
  ...
  req.body['username'] = req.body['username'].trim();
  let results = await BiometricModel.find({
    $and: [{ username: req.body['username'] }, { isDeleted: false }, { publicKey: req.body['publicKey'] }],
  });
...
async function updateBiometric(result, status, reason) {
  await BiometricModel.findByIdAndUpdate(result._id, {
    isDeleted: status,
    deleteReason: result == 'INACTIVE' ? 'huỷ do user không xác thực OTP' : reason,
  });
}
